...
# -*- coding: utf-8 -*-
"""
Multi-mesh 3DS → I3D UV patcher (numeric order)
-----------------------------------------------
For EVERY OBJECT_MESH in a .3ds file:
- Remove OBJECT_UV (0x4140)
- Create FACE_MAP_CHANNEL (0x4200) using the 0x4140 UV list and per-face vt indices
- Reorder ALL OBJECT_MESH subchunks by ascending numeric chunk ID (hex), including 0x4200
...
def patch_object_mesh_numeric(mesh_raw: bytes, channel: int) -> bytes:
    ...
    If no 0x4140 is present (no UVs), returns the original mesh blob unchanged.
    """
    ...
    # Gather chunks and parse what we need
    point = None
    faces_raw = None
    faces = None
    uvs = None
    preserved = []  # raw chunks EXCEPT 0x4140; faces will be added here too

    for cid, start, clen, body in kids:
        ...
        if cid == POINT_ARRAY:
            ...
            faces = faces_list
            preserved.append(raw)  # keep
        elif cid == OBJECT_UV_PRIMARY:
            # parse UVs, but DO NOT preserve raw 0x4140
            _, uv_list = parse_uvs(mesh_raw, body, start+clen)
            uvs = uv_list
        else:
            preserved.append(raw)  # keep all other subchunks verbatim

    # If there were no UVs, nothing to convert; return original mesh blob
    if uvs is None:
        return mesh_raw
    # If there are no faces or points, we cannot build per-triangle vt indices; return original
    if faces is None or point is None:
        return mesh_raw

    # Build 0x4200 and add it to the set
    fmc_chunk = write_chunk(FACE_MAP_CHANNEL, build_face_map_channel_payload(channel, uvs, faces))
    preserved.append(fmc_chunk)

    # Numeric sort ALL subchunks now present
...
def patch_file_numeric(src_path: Path, dst_path: Path, channel: int = 1):
    ...
    for cid, start, ln, body in oi_children:
...
def main():
    ap = argparse.ArgumentParser(description="Patch ALL OBJECT_MESH UVs (0x4140) into I3D FACE_MAP_CHANNEL (0x4200) with numeric subchunk ordering.")
